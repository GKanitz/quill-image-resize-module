...
var Resize = function (_BaseModule) {
    ...
    function Resize() {
        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Resize.__proto__ || Object.getPrototypeOf(Resize)).call.apply(_ref, [this].concat(args))), _this), _this.onCreate = function () {
            ...
            box.addEventListener('touchstart', _this.handleTouchstart, false);
            ...
            // handle the event
            _this.handleStart(evt.target, evt.clientX);
            ...
            // handle the event
            _this.handleStart(evt.target, evt.touches[0].screenX);
            // listen for movement and touchend
            document.addEventListener('touchmove', _this.handleTouchmove, false);
            document.addEventListener('touchend', _this.handleTouchend, false);
        }, _this.handleStart = function (target, x) {
            // note which box
            _this.dragBox = target;
            ...
            _this.handleMove(evt.clientX);
        }, _this.handleTouchmove = function (evt) {
            _this.handleMove(evt.touches[0].screenX);
        }, _this.handleMove = function (x) {
            if (!_this.img) {
                // image not set yet
                return;
            }
            // update image size
            var deltaX = x - _this.dragStartX;
